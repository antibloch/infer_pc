...
def run_semantic_segmentation(ply_path, output_folder="./output"):
    ...
    colors = np.asarray(pcd.colors, dtype=np.float32) * np.float32(255.)
    ...
    dummy_dataset = ml3d.datasets.Semantic3D(
        dataset_path="./dummy",
        use_cache=False
    )
    ...
    data = {
        ...
        'feat': colors,   # RGB colors as separate field (or None)
    ...
    o3d.io.write_point_cloud(os.path.join(output_folder, "segmented_semantic3d_randlanet.ply"), segmented_pcd)

    # Save the label dictionary
    with open(os.path.join(output_folder, "dic_semantic3d_randlanet.json"), "w") as f:
