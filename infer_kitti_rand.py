...
def run_semantic_segmentation(ply_path, output_folder="./output"):
    ...
    # Initialize model with original config (dim_input=3)
    model = ml3d.models.RandLANet(**cfg.model)
    
    # Create a simple dataset class for SemanticKITTI to get the label names
    ...
    # Use normalized points for inference
    data = {
        'point': points_normalized,     # Normalized XYZ coordinates
        'feat': None,
    ...
    segmented_pcd.points = o3d.utility.Vector3dVector(original_points)  # Use original points
    ...
    o3d.io.write_point_cloud(os.path.join(output_folder, "segmented_kitti_randlanet.ply"), segmented_pcd)

    # Save the label dictionary
    with open(os.path.join(output_folder, "dic_kitti_randlanet.json"), "w") as f:
