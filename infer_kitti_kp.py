...
def run_semantic_segmentation(ply_path, output_folder="./output"):
    ...
    # pcd = robust_downsample(pcd, method='voxel', target_factor=3, adaptive=True, preserve_features=True, max_iterations=5)
    # ---------------------Downsample ------------------------------------
    ...
    dummy_dataset = ml3d.datasets.SemanticKITTI(
        dataset_path="./dummy",
        use_cache=False
    )
    ...
    randlanet_url = "https://storage.googleapis.com/open3d-releases/model-zoo/kpconv_semantickitti_202009090354utc.pth"
    ...
    o3d.io.write_point_cloud(os.path.join(output_folder, "segmented_kitti_kp.ply"), segmented_pcd)

    # Save the label dictionary
    with open(os.path.join(output_folder, "dic_kitti_kp.json"), "w") as f:
