...
def robust_downsample(pcd, method='voxel', target_factor=10, adaptive=True, 
                      preserve_features=True, max_iterations=5):
    """
    A robust downsampling approach for point clouds with multiple methods,
    adaptive refinement, and feature preservation.
    
    Args:
        pcd: Open3D point cloud object
        method: Downsampling method ('voxel', 'uniform', 'farthest', 'poisson')
        target_factor: Target reduction factor
        adaptive: Whether to adaptively refine to hit target count more precisely
        preserve_features: Whether to preserve sharp features and edges
    ...
    target_points = original_points / target_factor
    
    # Check if we need to estimate normals for feature preservation or poisson sampling
    has_normals = pcd.has_normals()
    if (preserve_features or method == 'poisson') and not has_normals:
        print("Estimating normals for feature preservation...")
        pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(
                            radius=0.1, max_nn=30))
    
    # Feature detection (if preserve_features is True)
    feature_weights = None
    if preserve_features and has_normals:
        print("Detecting features...")
        # Compute curvature as a measure of "featureness"
        normals = np.asarray(pcd.normals)
        pcd_tree = o3d.geometry.KDTreeFlann(pcd)
        curvatures = []
        
        for i in range(len(pcd.points)):
            [_, idx, _] = pcd_tree.search_knn_vector_3d(pcd.points[i], 20)
            neighbor_normals = normals[idx]
            # Variance of normals as a simple curvature estimate
            curvature = 1 - np.abs(np.sum(neighbor_normals, axis=0) / len(idx))
            curvature = np.linalg.norm(curvature)
            curvatures.append(curvature)
        
        feature_weights = np.array(curvatures)
        feature_weights = feature_weights / np.max(feature_weights) if np.max(feature_weights) > 0 else feature_weights
    
    # Create a copy of the original for adaptive refinement
    result_pcd = pcd
    ...
    if method == 'voxel':
        ...
        point_density = original_points / bbox_volume
        voxel_size = (target_factor / point_density) ** (1/3)
        ...
        while iterations < max_iterations:
            result_pcd = pcd.voxel_down_sample(voxel_size=voxel_size)
            current_points = len(result_pcd.points)
            current_factor = original_points / current_points if current_points > 0 else float('inf')
            
            if not adaptive or abs(current_factor - target_factor) / target_factor < 0.1:
                # Within 10% of target, or adaptive is off
                break
                
            # Adjust voxel size based on result
            adjustment = (current_factor / target_factor) ** (1/3)
            voxel_size = voxel_size / adjustment
            iterations += 1
            
    elif method == 'uniform':
        # Calculate skipping stride for uniform sampling
        every_k_points = max(1, int(target_factor))
        result_pcd = pcd.uniform_down_sample(every_k_points=every_k_points)
        
    elif method == 'farthest':
        # Farthest point sampling
        try:
            num_samples = max(1, int(original_points / target_factor))
            result_pcd = pcd.farthest_point_down_sample(num_samples=num_samples)
        except RuntimeError as e:
            print(f"Error with farthest point sampling: {e}. Falling back to voxel.")
            return robust_downsample(pcd, 'voxel', target_factor, adaptive, preserve_features)
            
    elif method == 'poisson':
        # Poisson disk sampling
        try:
            # Initialize with a conservative radius estimate
            points_np = np.asarray(pcd.points)
            bbox_size = np.max(points_np, axis=0) - np.min(points_np, axis=0)
            avg_size = np.mean(bbox_size)
            radius = avg_size * (target_factor ** (1/3)) / 5  # Initial guess
            
            # Adaptive refinement for poisson
            iterations = 0
            while iterations < max_iterations:
                # Poisson disk sampling with current radius
                result_pcd = pcd.poisson_disk_sampling(
                    radius=radius,
                    use_triangle_normal=has_normals
                )
            ...
            print(f"Error with Poisson sampling: {e}. Falling back to voxel.")
            return robust_downsample(pcd, 'voxel', target_factor, adaptive, preserve_features)
    else:
        raise ValueError(f"Unknown method: {method}")
    
    # If we have feature weights and need to preserve features, we can bias toward keeping
    # points in high-feature regions by merging with a feature-weighted sampling
    if preserve_features and feature_weights is not None and method != 'farthest':
        # Take some points based on feature importance
        high_feature_indices = np.where(feature_weights > 0.7)[0]
        if len(high_feature_indices) > 0:
            # Create a point cloud of just the high-feature points
            feature_pcd = o3d.geometry.PointCloud()
            feature_pcd.points = o3d.utility.Vector3dVector(np.asarray(pcd.points)[high_feature_indices])
            if has_normals:
                feature_pcd.normals = o3d.utility.Vector3dVector(np.asarray(pcd.normals)[high_feature_indices])
            
            # Downsample the feature points (to avoid too many)
            feature_factor = max(1, int(len(high_feature_indices) / (target_points * 0.2)))
            if method == 'voxel':
                feature_pcd = feature_pcd.voxel_down_sample(voxel_size=voxel_size * 0.7)
            else:
                feature_pcd = feature_pcd.uniform_down_sample(every_k_points=feature_factor)
            
            # Merge the feature points with the regular downsampled points
            combined_points = np.vstack((np.asarray(result_pcd.points), np.asarray(feature_pcd.points)))
            combined_pcd = o3d.geometry.PointCloud()
            combined_pcd.points = o3d.utility.Vector3dVector(combined_points)
            
            # Remove duplicates with a very small voxel size
            small_voxel = voxel_size / 10 if 'voxel_size' in locals() else 0.001
            result_pcd = combined_pcd.voxel_down_sample(voxel_size=small_voxel)
    
    # Copy color and normal information if available
    if pcd.has_colors() and not result_pcd.has_colors():
        result_colors = np.zeros((len(result_pcd.points), 3))
        pcd_tree = o3d.geometry.KDTreeFlann(pcd)
        
        for i, point in enumerate(result_pcd.points):
            [_, idx, _] = pcd_tree.search_knn_vector_3d(point, 1)
            result_colors[i] = np.asarray(pcd.colors)[idx[0]]
        
        result_pcd.colors = o3d.utility.Vector3dVector(result_colors)
    
    # If original had normals but result doesn't, copy or recalculate
    if has_normals and not result_pcd.has_normals():
        result_pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(
                                  radius=0.1, max_nn=30))
    
    # Report results
    elapsed_time = time.time() - start_time
    final_points = len(result_pcd.points)
    final_factor = original_points / final_points if final_points > 0 else float('inf')
    
    print(f"Downsampling results:")
    print(f"  Method: {method}")
    print(f"  Original points: {original_points}")
    print(f"  Downsampled points: {final_points}")
    print(f"  Reduction factor: {final_factor:.2f}x (target: {target_factor:.2f}x)")
...
def downsample_point_cloud(pcd, target_reduction_factor=10):
    """
    Downsample a point cloud by approximately the specified reduction factor.
    
    Args:
        pcd: open3d point cloud object
        target_reduction_factor: desired reduction in number of points (e.g., 10 means 10x reduction)
    ...
    target_points = original_points / target_reduction_factor
    
    # Estimate voxel size based on point density and target reduction
    # This uses the cube root relationship between volume and point count
    bbox = pcd.get_axis_aligned_bounding_box()
    bbox_volume = np.prod(bbox.get_max_bound() - bbox.get_min_bound())
    point_density = original_points / bbox_volume
    
    # Calculate voxel size that should give the target reduction
    # Each voxel will contain roughly N points where N is the reduction factor
    voxel_size = (target_reduction_factor / point_density) ** (1/3)
    
    # Downsample the point cloud
    downsampled_pcd = pcd.voxel_down_sample(voxel_size=voxel_size)
    actual_reduction = original_points / len(downsampled_pcd.points)
    
    print(f"Downsampled point cloud has {len(downsampled_pcd.points)} points")
    print(f"Achieved reduction factor: {actual_reduction:.2f}x (target was {target_reduction_factor}x)")
...
def run_semantic_segmentation(ply_path, output_folder="./output"):
    """
    Run semantic segmentation on a PLY file using a pretrained RandLANet model.
    """
    ...
    # pcd = downsample_point_cloud(pcd, target_reduction_factor=2)
    # pcd = robust_downsample(pcd, method='voxel', target_factor=5, adaptive=True, preserve_features=True, max_iterations=5)
    # ---------------------Downsample ------------------------------------
    ...
    # Store original points for later
    original_points = points.copy()

    # # Center and scale normalization (closer to common approach for point cloud models)
    # # Using centroid normalization which is more standard for 3D point clouds
    ...
    # points_normalized = (points - min_point) / scale
    ...
    # Load configuration file with ORIGINAL settings
    # cfg_file = "ml3d/configs/randlanet_toronto3d.yml"
    cfg_file = "ml3d/configs/kpconv_toronto3d.yml"
    ...
    # create a simple dataset class for Semantic3D to get the label names
    dummy_dataset = ml3d.datasets.Toronto3D(
        dataset_path="./dummy",
        use_cache=False
    )

    label_to_names = dummy_dataset.label_to_names
    print("Label mapping:")
    for label_id, label_name in label_to_names.items():
    ...
    ckpt_path = ckpt_folder + "kpconv_toronto3d_202012221551utc.pth"
    # randlanet_url = "https://storage.googleapis.com/open3d-releases/model-zoo/randlanet_toronto3d_202201071330utc.pth"
    randlanet_url = "https://storage.googleapis.com/open3d-releases/model-zoo/kpconv_toronto3d_202012221551utc.pth"
    
    if not os.path.exists(ckpt_path):
        print(f"Downloading pretrained weights from {randlanet_url}")
    ...
    # Note: the model might actually expect unnormalized points, but we'll use normalized
    # as this is generally better for neural networks
    data = {
        'point': points_normalized,  # XYZ coordinates only (3D)
        'feat': None,   # RGB colors as separate field (or None)
    ...
    # Create a new point cloud with segmentation results
    segmented_pcd = o3d.geometry.PointCloud()
    segmented_pcd.points = o3d.utility.Vector3dVector(points_normalized)  # Use original points
    segment_colors = np.zeros((len(original_points), 3)).astype(np.float32)

    # Extract and adjust results based on reference code
    ...
    for i, label_id in enumerate(unique_labels):
        if label_id in label_to_names:

            rand_color = np.random.randint(0, 255, 3)
            color_name = get_closest_color_name(rand_color[0], rand_color[1], rand_color[2])

            print(f"Class {label_id} ({label_to_names[label_id]}): {counts[i]} points ({counts[i]/len(predicted_labels)*100:.2f}%) | Mapped to {color_name} : [rgb({rand_color[0]},{rand_color[1]},{rand_color[2]})]███[/]")
            segment_colors[predicted_labels==label_id] = rand_color

            color_tuple = rand_color.tolist()
            label_dict[label_to_names[label_id]] = color_tuple
                
        else:
            print(f"Unknown class {label_id}: {counts[i]} points ({counts[i]/len(predicted_labels)*100:.2f}%)")

    segmented_pcd.colors = o3d.utility.Vector3dVector(segment_colors / 255.0)
    # save the segmented point cloud
    o3d.io.write_point_cloud(os.path.join(output_folder, "segmented_toronto_kp.ply"), segmented_pcd)

    # Save the label dictionary
    with open(os.path.join(output_folder, "dic_toronto_kp.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run semantic segmentation on a PLY file")
    ...
    run_semantic_segmentation(args.ply, args.output)
